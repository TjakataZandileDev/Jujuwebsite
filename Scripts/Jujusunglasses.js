...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  window.addToCart = (dropdownId, itemName) => {
    const quantity = parseInt(document.querySelector(`#${dropdownId} .quantity-btn`).textContent);
    const price = parseInt(document.querySelector(`img[alt="${itemName}"]`).getAttribute('data-price'));
  ...
  window.displayCartItems = () => {
    ...
    cart.forEach(item => {
      ...
      itemElement.textContent = `${item.itemName} - Quantity: ${item.quantity} - Price: R${item.price}`;
  ...
  window.selectQuantity = (quantity, dropdownId) => {
    document.querySelector(`#${dropdownId} .quantity-btn`).textContent = `${quantity} \u25BC`;
    document.getElementById(dropdownId).classList.remove('show');
  };

  window.selectLocation = (location, dropdownId) => {
